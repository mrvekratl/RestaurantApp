...
namespace RestaurantApp.Services.Repositories
{
    public class SaleRepository : ISaleRepository
    {
        ...
        public async Task<IEnumerable<Sale>> GetWeeklySalesAsync()
        {
            ...
            return await _context.Sales
                .Where(s => s.SaleDate >= startOfWeek && s.SaleDate <= DateTime.Today)
                .Include(s => s.SaleProducts)
                .ThenInclude(sp => sp.Product)
                .ToListAsync();
        }
        public async Task<IEnumerable<Sale>> GetMonthlySalesAsync()
        {
            ...
            return await _context.Sales
                .Where(s => s.SaleDate >= startOfMonth && s.SaleDate <= DateTime.Today)
                .Include(s => s.SaleProducts)
                .ThenInclude(sp => sp.Product)
                .ToListAsync();
        }
        ...
        {
            await _context.SaleProducts.AddAsync(saleProducts);
            await _context.SaveChangesAsync();
        }
        ...
        {
            return await _context.Sales
               .Where(s => s.SaleDate == date && s.SaleProducts.Any(sp => sp.UserId == userId))
               .Include(s => s.SaleProducts)
               .ThenInclude(sp => sp.Product)
               .ToListAsync();
        }
